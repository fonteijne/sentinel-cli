...
def _format_decision_log(ticket_id: str, iteration: int, dev_result: dict, sec_result: dict) -> str:
    """Format the decision log comment for the GitLab MR.
    
    Creates a markdown-formatted summary of the execution results for review
    by developers reading the MR.
    
    Args:
        ticket_id: Jira ticket identifier
        iteration: The iteration number where security approved
        dev_result: Developer agent execution result dict
        sec_result: Security reviewer agent result dict
    ...
    # Show success status if all tasks passed
    if dev_result['tasks_failed'] == 0 and dev_result['tasks_completed'] > 0:
        lines.append("- **All tasks passed** ✅")
    
    # Include test results if available
    if dev_result.get('test_results'):
        test_result = dev_result['test_results']
        if test_result.get('passed'):
            lines.append(f"- **Tests:** {test_result['passed']} passed")
        if test_result.get('failed'):
            lines.append(f"- **Test failures:** {test_result['failed']}")
    ...
    if sec_result.get('findings'):
        findings_count = len(sec_result['findings'])
        lines.append(f"- **Issues Found & Fixed:** {findings_count}")
    else:
    ...
    lines.extend([
        "",
        "---",
        f"*Generated by Sentinel on {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}*"
    ])
